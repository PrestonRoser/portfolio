...
| `npm run previews` | Re-capture project site previews in `src/assets/previews/` (needs Chrome) |
...
- `src/assets/previews`: site screenshots shown on project pages that have a live site or repo,
  generated by `npm run previews` and committed. A `.png` or `.jpg` named after the project
